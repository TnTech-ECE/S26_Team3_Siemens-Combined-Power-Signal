...
<img width="1075" height="414" alt="image" src=""/>
...
### Communication
There are two system-level approaches to consider for the communications for the product. One approach uses a Power Line Communications modem (PLC modem) with a programmable ARM processor to process input data and through a program to then send to the data link and physical layers. These layers create a modulated packet signal to send over the transmission line. The other approach would be to use a PLC modem without an SoC to allow inputs to interface with the data link and physical layers directly.
